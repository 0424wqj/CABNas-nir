...
def process_spectrum_data(file_path):
    ...
    # 6. 使用GridSearchCV进行MLP训练和参数优化（使用全部特征）
    print("正在进行MLP训练和参数优化（使用全部特征）...")
    param_grid = {
    ...
    print(f"特征选择方法: 无")
    print(f"网络结构: {best_model.hidden_layer_sizes}")
    print(f"训练迭代次数: {best_model.n_iter_}")
    ...
    return {
        ...
        'method': 'MLP-无特征选择',
        'network_structure': best_model.hidden_layer_sizes,
