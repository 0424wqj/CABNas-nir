...
class IntensityScalingAugmentation:
    """强度缩放数据增强器"""
    ...
    def intensity_scaling(self, spectrum, scale_range=(0.8, 1.2)):
        """
        强度缩放
        模拟样品浓度变化或光程变化
        """
    ...
    def augment_data(self, X, y, augmentation_factor=3, scale_range=(0.8, 1.2)):
        """
        对数据进行强度缩放增强
        """
        print(f"📏 正在进行强度缩放数据增强...")
        ...
        for i in range(augmentation_factor):
            X_aug = np.array([self.intensity_scaling(spectrum, scale_range) for spectrum in X])
...
def process_spectrum_data_with_intensity_scaling_augmentation(file_path):
    """
    CARS特征选择 + 强度缩放数据增强 + AutoKeras CNN+RNN
    """
    print("="*80)
    print("        CARS + 强度缩放增强 + AutoKeras CNN+RNN")
    print("="*80)
    ...
    # 7. 强度缩放数据增强
    print("\n步骤7: 正在进行强度缩放数据增强...")
    augmenter = IntensityScalingAugmentation(random_state=42)
    ...
    # 打印网络结构
    print_model_architecture(model, "CARS + 强度缩放增强 + AutoKeras CNN+RNN")
    ...
    print("\n" + "="*80)
    print("       CARS + 强度缩放增强 + AutoKeras 模型评估结果")
    print("="*80)
...
if __name__ == '__main__':
    ...
    print("📏 启动CARS + 强度缩放增强 + AutoKeras CNN+RNN实验...")
    if gpu_available:
        print("🚀 使用GPU加速训练")
    
    try:
        result = process_spectrum_data_with_intensity_scaling_augmentation(data_path)
