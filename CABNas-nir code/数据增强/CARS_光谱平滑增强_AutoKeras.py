...
class SpectralSmoothingAugmentation:
    """光谱平滑数据增强器"""
    ...
    def spectral_smoothing(self, spectrum, window_length=5, polyorder=2):
        """
        光谱平滑
        模拟不同的预处理效果
        """
        # 随机选择窗口长度和多项式阶数
        window_choices = [3, 5, 7, 9, 11]
        poly_choices = [1, 2, 3]
        
        window = np.random.choice(window_choices)
        poly = np.random.choice(poly_choices)
        
        # 确保窗口长度为奇数且不超过数据长度
        if window % 2 == 0:
            window += 1
        window = min(window, len(spectrum) - 1)
        if window % 2 == 0:
            window -= 1
    ...
    def augment_data(self, X, y, augmentation_factor=3):
        ...
        对数据进行光谱平滑增强
        """
        print(f"🌊 正在进行光谱平滑数据增强...")
        print(f"   增强倍数: {augmentation_factor}")
        print(f"   平滑方法: Savitzky-Golay滤波器")
        
        X_augmented_list = [X]  # 包含原始数据
        y_augmented_list = [y]
        
        for i in range(augmentation_factor):
            X_aug = np.array([self.spectral_smoothing(spectrum) for spectrum in X])
...
def process_spectrum_data_with_smoothing_augmentation(file_path):
    """
    CARS特征选择 + 光谱平滑数据增强 + AutoKeras CNN+RNN
    """
    print("="*80)
    print("        CARS + 光谱平滑增强 + AutoKeras CNN+RNN")
    print("="*80)
    ...
    # 7. 光谱平滑数据增强
    print("\n步骤7: 正在进行光谱平滑数据增强...")
    augmenter = SpectralSmoothingAugmentation(random_state=42)
    ...
    # 打印网络结构
    print_model_architecture(model, "CARS + 光谱平滑增强 + AutoKeras CNN+RNN")
    ...
    print("\n" + "="*80)
    print("       CARS + 光谱平滑增强 + AutoKeras 模型评估结果")
    print("="*80)
    ...
    print(f"增强方法: 光谱平滑 (Savitzky-Golay滤波)")
...
if __name__ == '__main__':
    ...
    print("🌊 启动CARS + 光谱平滑增强 + AutoKeras CNN+RNN实验...")
    if gpu_available:
        print("🚀 使用GPU加速训练")
    
    try:
        result = process_spectrum_data_with_smoothing_augmentation(data_path)
