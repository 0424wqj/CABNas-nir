...
class BaselineDriftAugmentation:
    ...
    def add_baseline_drift(self, spectrum, drift_strength=0.05):
        ...
        # 确保有足够的控制点进行插值
        min_points = max(4, min(6, n_points // 10))  # 最少4个点，最多根据数据长度调整
        max_points = min(10, n_points // 5)          # 最多10个点
        ...
        # 确保边界点的漂移较小
        control_y[0] *= 0.5  # 起始点漂移减半
        control_y[-1] *= 0.5  # 结束点漂移减半
...
def process_spectrum_data_with_baseline_drift_augmentation(file_path):
    """
    CARS特征选择 + 基线漂移数据增强 + AutoKeras CNN+RNN
    """
    print("="*80)
    print("        CARS + 基线漂移增强 + AutoKeras CNN+RNN")
    print("="*80)
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X_sg, y_encoded, test_size=0.5, random_state=54, stratify=y_encoded
    )
    ...
    # 打印网络结构
    print_model_architecture(model, "CARS + 基线漂移增强 + AutoKeras CNN+RNN")
    ...
    print("\n" + "="*80)
    print("       CARS + 基线漂移增强 + AutoKeras 模型评估结果")
    print("="*80)
...
if __name__ == '__main__':
    ...
    print("📈 启动CARS + 基线漂移增强 + AutoKeras CNN+RNN实验...")
    if gpu_available:
        print("🚀 使用GPU加速训练")
    
    try:
        result = process_spectrum_data_with_baseline_drift_augmentation(data_path)
