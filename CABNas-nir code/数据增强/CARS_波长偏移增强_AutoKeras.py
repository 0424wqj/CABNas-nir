...
class WavelengthShiftAugmentation:
    """波长偏移数据增强器"""
    ...
    def wavelength_shift(self, spectrum, shift_range=(-2, 2)):
        """
        波长偏移
        模拟仪器波长校准误差
        """
        n_points = len(spectrum)
        shift_pixels = np.random.uniform(shift_range[0], shift_range[1])
        
        # 创建新的索引
        old_indices = np.arange(n_points)
        new_indices = old_indices + shift_pixels
        
        # 边界处理
        new_indices = np.clip(new_indices, 0, n_points - 1)
        
        # 插值
        f = interp1d(old_indices, spectrum, kind='linear', 
                    bounds_error=False, fill_value='extrapolate')
        return f(new_indices)
    
    def augment_data(self, X, y, augmentation_factor=3, shift_range=(-2, 2)):
        """
        对数据进行波长偏移增强
        """
        print(f"〰️ 正在进行波长偏移数据增强...")
        print(f"   增强倍数: {augmentation_factor}")
        print(f"   偏移范围: {shift_range} 像素")
        
        X_augmented_list = [X]  # 包含原始数据
        y_augmented_list = [y]
        
        for i in range(augmentation_factor):
            X_aug = np.array([self.wavelength_shift(spectrum, shift_range) for spectrum in X])
...
def process_spectrum_data_with_wavelength_shift_augmentation(file_path):
    """
    CARS特征选择 + 波长偏移数据增强 + AutoKeras CNN+RNN
    """
    print("="*80)
    print("        CARS + 波长偏移增强 + AutoKeras CNN+RNN")
    print("="*80)
    ...
    # 打印网络结构
    print_model_architecture(model, "CARS + 波长偏移增强 + AutoKeras CNN+RNN")
    ...
    print("\n" + "="*80)
    print("       CARS + 波长偏移增强 + AutoKeras 模型评估结果")
    print("="*80)
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', 
                xticklabels=[str(label) for label in le.classes_],
                yticklabels=[str(label) for label in le.classes_])
    ...
    # 调整布局并保存
    plt.tight_layout()
    plt.savefig('wavelength_shift_confusion_matrix.png', dpi=600)
    plt.show()
    print("✅ 混淆矩阵已保存为 wavelength_shift_confusion_matrix.png")
...
if __name__ == '__main__':
    ...
    print("〰️ 启动CARS + 波长偏移增强 + AutoKeras CNN+RNN实验...")
    if gpu_available:
        print("🚀 使用GPU加速训练")
    
    try:
        result = process_spectrum_data_with_wavelength_shift_augmentation(data_path)
