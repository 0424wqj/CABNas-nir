...
class LeastConfidentActiveLearning:
    """最小置信度主动学习采样器"""
    ...
    def least_confident_sampling(self, model, X_pool, n_samples=100):
        """
        最小置信度采样策略
        选择模型预测置信度最低的样本
        """
        print(f"🎯 执行最小置信度采样，选择 {n_samples} 个样本")
        ...
        try:
            ...
            avg_confidence = np.mean(confidences[selected_indices])
            avg_uncertainty = np.mean(uncertainties[selected_indices])
            min_confidence = np.min(confidences[selected_indices])
            max_confidence = np.max(confidences[selected_indices])
            
            print(f"   平均置信度: {avg_confidence:.4f}")
            print(f"   平均不确定性: {avg_uncertainty:.4f}")
            print(f"   最小置信度: {min_confidence:.4f}")
            print(f"   最大置信度: {max_confidence:.4f}")
            print(f"   置信度标准差: {np.std(confidences[selected_indices]):.4f}")
            
            return selected_indices, confidences[selected_indices]
            
        except Exception as e:
            print(f"⚠️ 最小置信度采样失败: {e}")
    ...
    def active_learning_selection(self, X_augmented, y_augmented, n_samples=200, 
        ...
        if len(X_pool) <= n_samples - len(X_initial):
            ...
            selected_confidences = np.array([])
        else:
            selected_pool_indices, selected_confidences = self.least_confident_sampling(
                initial_model, X_pool, n_samples - len(X_initial)
            )
        ...
        return X_selected, y_selected, final_indices, selected_confidences
...
def process_spectrum_data_with_least_confident_active_learning(file_path):
    """
    CARS特征选择 + 基线漂移数据增强 + 最小置信度主动学习 + AutoKeras CNN+RNN
    """
    print("="*80)
    print("  CARS + 基线漂移增强 + 最小置信度主动学习 + AutoKeras CNN+RNN")
    print("="*80)
    ...
    # 8. 最小置信度主动学习采样
    print("\n步骤8: 正在进行最小置信度主动学习采样...")
    active_learner = LeastConfidentActiveLearning(random_state=42)
    X_selected, y_selected, selected_indices, confidences = active_learner.active_learning_selection(
    ...
    # 打印网络结构
    print_model_architecture(model, "CARS + 基线漂移增强 + 最小置信度主动学习 + AutoKeras CNN+RNN")
    ...
    print("\n" + "="*80)
    print(" CARS + 基线漂移增强 + 最小置信度主动学习 + AutoKeras 模型评估结果")
    print("="*80)
    ...
    if len(confidences) > 0:
        print(f"选择样本平均置信度: {np.mean(confidences):.4f}")
...
if __name__ == '__main__':
    ...
    print("📈 启动CARS + 基线漂移增强 + 最小置信度主动学习 + AutoKeras CNN+RNN实验...")
    if gpu_available:
        print("🚀 使用GPU加速训练")
    
    try:
        result = process_spectrum_data_with_least_confident_active_learning(data_path)
