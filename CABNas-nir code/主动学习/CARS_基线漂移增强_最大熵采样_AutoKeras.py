...
class MaxEntropyActiveLearning:
    """最大熵主动学习采样器"""
    ...
    def entropy_sampling(self, model, X_pool, n_samples=50):
        """
        最大熵采样策略
        选择预测分布熵最大的样本
        """
        print(f"🎯 执行最大熵采样，选择 {n_samples} 个样本")
        ...
        try:
            ...
            avg_entropy = np.mean(entropies[selected_indices])
            max_entropy = np.max(entropies[selected_indices])
            min_entropy = np.min(entropies[selected_indices])
            
            print(f"   平均熵: {avg_entropy:.4f}")
            print(f"   最大熵: {max_entropy:.4f}")
            print(f"   最小熵: {min_entropy:.4f}")
            print(f"   熵标准差: {np.std(entropies[selected_indices]):.4f}")
            
            return selected_indices, entropies[selected_indices]
    ...
    def active_learning_selection(self, X_augmented, y_augmented, n_samples=200, 
        ...
        if len(X_pool) <= n_samples - len(X_initial):
            ...
            selected_entropies = np.array([])
        else:
            selected_pool_indices, selected_entropies = self.entropy_sampling(
        ...
        return X_selected, y_selected, final_indices, selected_entropies
...
def process_spectrum_data_with_max_entropy_active_learning(file_path):
    """
    CARS特征选择 + 基线漂移数据增强 + 最大熵主动学习 + AutoKeras CNN+RNN
    """
    print("="*80)
    print("   CARS + 基线漂移增强 + 最大熵主动学习 + AutoKeras CNN+RNN")
    print("="*80)
    ...
    # 8. 最大熵主动学习采样
    print("\n步骤8: 正在进行最大熵主动学习采样...")
    active_learner = MaxEntropyActiveLearning(random_state=42)
    X_selected, y_selected, selected_indices, entropies = active_learner.active_learning_selection(
    ...
    # 打印网络结构
    print_model_architecture(model, "CARS + 基线漂移增强 + 最大熵主动学习 + AutoKeras CNN+RNN")
    ...
    print("\n" + "="*80)
    print("  CARS + 基线漂移增强 + 最大熵主动学习 + AutoKeras 模型评估结果")
    print("="*80)
    ...
    if len(entropies) > 0:
        print(f"选择样本平均熵: {np.mean(entropies):.4f}")
    ...
    return {
        ...
        'avg_entropy': np.mean(entropies) if len(entropies) > 0 else 0
    }
...
if __name__ == '__main__':
    ...
    print("📈 启动CARS + 基线漂移增强 + 最大熵主动学习 + AutoKeras CNN+RNN实验...")
    if gpu_available:
        print("🚀 使用GPU加速训练")
    
    try:
        result = process_spectrum_data_with_max_entropy_active_learning(data_path)
