...
class Solution:
    ...
    def get_neighbor(self):
        """生成邻域解：随机翻转1-5个特征的选择状态"""
        neighbor = self.copy()
        
        # 随机选择要翻转的特征数量（1-5个，增加变化幅度）
        n_flips = random.randint(1, min(5, self.n_features))
...
class SimulatedAnnealing:
    ...
    def fitness_function(self, X, y, solution):
        ...
        feature_penalty = feature_ratio * 0.05  # 增加惩罚力度
...
def process_spectrum_data(file_path):
    ...
    sa = SimulatedAnnealing(
        initial_temperature=50,      # 初始温度（降低以提高效率）
        final_temperature=0.01,      # 最终温度
        cooling_rate=0.95,           # 冷却率
        max_iterations=500,          # 最大迭代数（减少以提高速度）
        cv_folds=3                   # 交叉验证折数（减少以提高速度）
    )
    ...
    # 6. 使用GridSearchCV进行MLP模型训练和参数优化（使用SA选择的特征）
    print("开始MLP训练和参数优化...")
    param_grid = {
