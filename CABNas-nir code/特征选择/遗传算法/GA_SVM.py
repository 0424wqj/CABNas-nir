...
class GeneticAlgorithm:
    """遗传算法特征选择"""
    def __init__(self, population_size=50, generations=10, crossover_rate=0.8,
    ...
    def fitness_function(self, X, y, individual):
        ...
        for train_idx, val_idx in kf.split(X_selected):
            ...
            y_pred_cv = svm.predict(X_val_cv)
...
def process_spectrum_data(file_path):
    ...
    # 2. SG预处理
    X_sg = savgol_filter(X, window_length=7, polyorder=2, axis=1)
    ...
    # 6. 使用GridSearchCV进行SVM模型训练和参数优化（使用GA选择的特征）
    print("开始SVM训练和参数优化...")
    param_grid = {
        'C': [0.1, 1, 10, 100],
