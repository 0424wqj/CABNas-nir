...
class PSO:
    """粒子群优化特征选择算法"""
    def __init__(self, n_particles=30, n_iterations=10, w=0.5, c1=2.0, c2=2.0, cv_folds=5):
    ...
    def fitness_function(self, X, y, feature_mask):
        ...
        for train_idx, val_idx in kf.split(X_selected):
            ...
            y_pred_cv = mlp.predict(X_val_cv)
...
def process_spectrum_data(file_path):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X_sg, y, test_size=0.3, random_state=42, stratify=y
    )
    ...
    # 5. PSO特征选择
    print("开始粒子群算法特征选择...")
    pso = PSO(n_particles=30, n_iterations=30, w=0.5, c1=2.0, c2=2.0, cv_folds=5)
    ...
    # 6. 使用GridSearchCV进行MLP模型训练和参数优化（使用PSO选择的特征）
    print("开始MLP训练和参数优化...")
    param_grid = {
    ...
    grid_search = GridSearchCV(
        MLPClassifier(max_iter=1000, random_state=42, early_stopping=True, validation_fraction=0.1),
        param_grid, 
        cv=3, 
        n_jobs=-1
    )
